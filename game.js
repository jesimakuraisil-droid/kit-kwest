...
const BIOMES = ["meadow", "desert", "snow", "cave", "candy", "cloud", "hollow", "underwater", "volcano"];
function biomeColors(biome) {
  switch (biome) {
    ...
    case "cloud": return { sky1: "#eaf4ff", sky2: "#d0e8ff", ground: "#f5f0ff", accent: "#c9b8f0" };
    case "hollow": return { sky1: "#16262a", sky2: "#0a1417", ground: "#1f3a3d", accent: "#2ee6c7" };
    case "underwater": return { sky1: "#2f5f8c", sky2: "#123258", ground: "#d4c199", accent: "#3fb5a3" };
    case "volcano": return { sky1: "#5c2418", sky2: "#2a0f0a", ground: "#2a1a1a", accent: "#ff7043" };
...
const BIOME_AMBIENCE = {
  ...
  cloud: { type: "highpass", freq: 1800, Q: 0.4, gain: 0.07 },   // airy shimmer
  hollow: { type: "lowpass", freq: 120, Q: 1.2, gain: 0.06 },    // deep sub-bass hum
  underwater: { type: "lowpass", freq: 350, Q: 1.5, gain: 0.13 }, // muffled, gurgly
  volcano: { type: "lowpass", freq: 220, Q: 0.9, gain: 0.15 },   // rumbling drone
};
...
function applyBiomeAmbience(biome) {
  ...
  ambientGain.gain.setTargetAtTime(soundMuted ? 0 : cfg.gain * ambientVolumeMult, now, 0.6);
...
function playDrip() {
  ...
  g.gain.exponentialRampToValueAtTime(0.16 * ambientVolumeMult, now + 0.01);
...
// ---------------------------------------------------------------------------
// Gameplay SFX — small synthesized blips for feedback on key actions.
// ---------------------------------------------------------------------------
function playTone(freq, startTime, duration, { type = "sine", peak = 0.18, glideTo = null } = {}) {
  if (!audioCtx || soundMuted) return;
  const osc = audioCtx.createOscillator();
  const g = audioCtx.createGain();
  osc.type = type;
  ...
  g.gain.exponentialRampToValueAtTime(Math.max(0.0002, peak * sfxVolumeMult), startTime + Math.min(0.02, duration * 0.2));
...
function playJumpSound() {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  playTone(420, now, 0.14, { type: "square", peak: 0.09, glideTo: 700 });
}
function playLandSound() {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  playTone(180, now, 0.08, { type: "triangle", peak: 0.08, glideTo: 90 });
}
function playCorrectSound() {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  playTone(523, now, 0.1, { type: "sine", peak: 0.16 });
  playTone(784, now + 0.08, 0.16, { type: "sine", peak: 0.18 });
}
function playWrongSound() {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  playTone(300, now, 0.3, { type: "sawtooth", peak: 0.1, glideTo: 90 });
}
function playLevelUpSound() {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  [523, 659, 784, 1047].forEach((f, i) => playTone(f, now + i * 0.09, 0.16, { type: "sine", peak: 0.16 }));
}
function playBossHitSound() {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  playTone(150, now, 0.12, { type: "square", peak: 0.16, glideTo: 60 });
}
function playCoinSound() {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  playTone(1200, now, 0.09, { type: "sine", peak: 0.12, glideTo: 1600 });
}
function playPortalSound() {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  playTone(600, now, 0.18, { type: "triangle", peak: 0.12, glideTo: 900 });
}

// --- Per-biome ambient accents — occasional one-off sounds layered on top
// of the continuous bed, so each biome feels alive rather than a flat drone.
let accentTimeoutId = null;
function scheduleAmbientAccent() {
  const delay = 3000 + Math.random() * 5000;
  accentTimeoutId = setTimeout(() => {
    playAmbientAccent(effectiveBiome);
    scheduleAmbientAccent();
  }, delay);
}
function playAmbientAccent(biome) {
  if (!audioCtx || soundMuted) return;
  const now = audioCtx.currentTime;
  if (biome === "meadow") {
    // bird chirp
    playTone(1800 + Math.random() * 600, now, 0.06, { type: "sine", peak: 0.05, glideTo: 2400 });
    playTone(1600 + Math.random() * 600, now + 0.09, 0.06, { type: "sine", peak: 0.05, glideTo: 2200 });
  } else if (biome === "desert") {
    // faint gust swell
    playTone(200, now, 1.2, { type: "sawtooth", peak: 0.02, glideTo: 140 });
  } else if (biome === "snow") {
    // wind chime tinkle
    [1400, 1700, 2100].forEach((f, i) => playTone(f, now + i * 0.05, 0.3, { type: "sine", peak: 0.04 }));
  } else if (biome === "candy") {
    // soft bell
    playTone(1000, now, 0.35, { type: "sine", peak: 0.05 });
  } else if (biome === "castle") {
    // distant echoing clang
    playTone(220, now, 0.5, { type: "square", peak: 0.03, glideTo: 140 });
  } else if (biome === "cloud") {
    // magical shimmer sparkle
    [2200, 2600, 3000].forEach((f, i) => playTone(f, now + i * 0.06, 0.2, { type: "sine", peak: 0.03 }));
  } else if (biome === "hollow") {
    // eerie deep echo
    playTone(90, now, 0.9, { type: "sine", peak: 0.05, glideTo: 60 });
  } else if (biome === "underwater") {
    // bubble blip
    playTone(500 + Math.random() * 300, now, 0.12, { type: "sine", peak: 0.05, glideTo: 900 });
  } else if (biome === "volcano") {
    // crackle/pop
    playTone(140, now, 0.15, { type: "sawtooth", peak: 0.07, glideTo: 80 });
  }
}
...
function updatePlayer() {
  ...
  // On touch devices the player's thumbs and the on-screen controls sit
  // over the bottom corners of the screen, so keep the cat further left
  // and reveal more of the incoming path to compensate.
  const screenX = player.x - camX;
  const deadLeft = isTouchDevice ? W * 0.2 : W * 0.28;
  const deadRight = isTouchDevice ? W * 0.48 : W * 0.6;
  ...
  for (const sp of sparkles) {
    sp.timer--;
    if (sp.vx !== undefined) { sp.x += sp.vx; sp.y += sp.vy; sp.vy += 0.15; }
  }
...
function startBoss(isFinal) {
  ...
  const species = ["blob", "bat", "golem"];
  const type = isFinal ? "castle" : species[Math.floor(Math.random() * species.length)];
  const baseY = GROUND_Y - (isFinal ? 90 : type === "golem" ? 70 : 60);
  boss = {
    arenaX,
    isFinal: !!isFinal,
    type,
    x: arenaX + 200, y: baseY, baseY,
    w: isFinal ? 110 : type === "golem" ? 84 : 70,
    h: isFinal ? 100 : type === "golem" ? 74 : 60,
    minX: arenaX + 60, maxX: arenaX + (isFinal ? 460 : 340),
    speed: isFinal ? 2.8 : type === "golem" ? 1.5 : type === "bat" ? 2.8 : 2.2,
    dir: 1,
    hp: isFinal ? 15 : type === "golem" ? 7 : 5,
    maxHp: isFinal ? 15 : type === "golem" ? 7 : 5,
    hitCooldown: 0,
    spawnTimer: 80,
    projectiles: [],
    attacks: [],
...
function updateBoss() {
  ...
  if (boss.type === "bat") {
    boss.y = boss.baseY + Math.sin(frameCount * 0.06) * 24;
  }
  if (!boss.enraged && boss.hp <= boss.maxHp * 0.3) {
    boss.enraged = true;
    boss.speed *= 1.4;
    showToast("The boss is enraged!");
  }
  ...
  for (const atk of boss.attacks) {
    ...
    if (Math.hypot(dx, dy) < 30 && boss.hitCooldown === 0) {
      ...
      atk.hit = true;
      playBossHitSound();
      spawnBurst(boss.x + boss.w / 2, boss.y + boss.h / 2, "255,255,255", 10);
      triggerShake(4, 10);
      if (boss.hp <= 0) {
        defeatBoss();
        return;
  ...
  if (boss.spawnTimer <= 0) {
    boss.spawnTimer = Math.max(30, (90 - boss.maxHp - (boss.maxHp - boss.hp) * 10) / (boss.enraged ? 1.6 : 1));
...
function defeatBoss() {
  ...
  triggerShake(wasFinal ? 10 : 6, 20);
  spawnBurst(boss.x + boss.w / 2, boss.y + boss.h / 2, "255,213,79", 24);
  if (wasFinal) {
...
function drawBackground(biome) {
  ...
  // distant mountain silhouettes — slow parallax, sits behind the near hills
  const farColor = shadeColor(c.accent, 22);
  ctx.fillStyle = farColor.replace("rgb", "rgba").replace(")", ",0.55)");
  const peakW = 160;
  const farOff = -((camX * 0.15) % peakW);
  for (let i = -1; i * peakW + farOff < W + peakW; i++) {
    const px = i * peakW + farOff;
    const ph = 60 + ((i + Math.floor(camX / peakW)) % 4) * 22;
    ctx.beginPath();
    ctx.moveTo(px, GROUND_Y);
    ctx.lineTo(px + peakW * 0.5, GROUND_Y - ph);
    ctx.lineTo(px + peakW, GROUND_Y);
    ctx.closePath();
    ctx.fill();
  }
...
function drawGroundTexture(x0, x1, biome) {
  ...
  for (let gx = x0; gx < x1; gx += sliceW) {
    ...
    const sx = gx - camX;
    const hash = Math.abs(Math.sin(gx * 0.137)) % 1;
    ...
    if (biome === "cave") {
      ...
      if (hash > 0.75) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(sx + 2, GROUND_Y + 22);
        ctx.lineTo(sx + 10, GROUND_Y + 12);
        ctx.lineTo(sx + 16, GROUND_Y + 24);
        ctx.stroke();
      }
    } else if (biome === "snow") {
      ...
      if (hash > 0.7) {
        ...
        ctx.fillRect(sx + 8, GROUND_Y - 2, 2, 2);
      }
    } else if (biome === "desert") {
      ...
      if (hash > 0.8) {
        ...
        ctx.beginPath();
        ctx.arc(sx + 10, GROUND_Y + 6, 3, 0, Math.PI * 2);
        ctx.fill();
      }
    } else if (biome === "candy") {
      ...
      if (hash > 0.7) {
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(sx + 10, GROUND_Y + 6, 4, 0, Math.PI * 1.4);
        ctx.stroke();
      }
    } else if (biome === "castle") {
      ctx.strokeStyle = "rgba(0,0,0,0.35)";
      ctx.lineWidth = 1;
      const brickRow = Math.floor(gx / sliceW) % 2;
      ctx.strokeRect(sx + brickRow * 10 - 10, GROUND_Y + 8, sliceW, 10);
      if (hash > 0.85) {
        ctx.fillStyle = "rgba(255,140,60,0.5)";
        ctx.fillRect(sx + 8, GROUND_Y + 2, 3, 3);
      }
    } else if (biome === "cloud") {
      ctx.fillStyle = "rgba(255,255,255,0.8)";
      ctx.beginPath();
      ctx.arc(sx + 6, GROUND_Y + 4, 5, 0, Math.PI * 2);
      ctx.arc(sx + 14, GROUND_Y + 5, 4, 0, Math.PI * 2);
      ctx.fill();
      if (hash > 0.8) {
        ctx.fillStyle = "rgba(255,235,150,0.7)";
        ctx.beginPath();
        ctx.arc(sx + 10, GROUND_Y - 2, 2, 0, Math.PI * 2);
        ctx.fill();
      }
    } else if (biome === "hollow") {
      ctx.fillStyle = `rgba(46,230,199,${0.3 + hash * 0.3})`;
      ctx.beginPath();
      ctx.arc(sx + 10, GROUND_Y + 4, 2 + hash * 2, 0, Math.PI * 2);
      ctx.fill();
      if (hash > 0.75) {
        ctx.fillStyle = "#2ee6c7";
        ctx.fillRect(sx + 8, GROUND_Y - 8, 3, 8);
        ctx.beginPath();
        ctx.arc(sx + 9.5, GROUND_Y - 8, 4, 0, Math.PI * 2);
        ctx.fill();
      }
    } else if (biome === "underwater") {
      ctx.fillStyle = "rgba(255,255,255,0.35)";
      const bob = (frameCount * 0.4 + gx) % 40;
      ctx.beginPath();
      ctx.arc(sx + 10, GROUND_Y - bob, 2, 0, Math.PI * 2);
      ctx.fill();
      if (hash > 0.8) {
        ctx.fillStyle = "#ff8fa3";
        ctx.beginPath();
        ctx.arc(sx + 10, GROUND_Y + 4, 3, 0, Math.PI * 2);
        ctx.fill();
      }
    } else if (biome === "volcano") {
      ctx.strokeStyle = `rgba(255,112,67,${0.4 + 0.3 * Math.sin(frameCount * 0.1 + gx)})`;
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(sx + 2, GROUND_Y + 4);
      ctx.lineTo(sx + 10, GROUND_Y + 12);
      ctx.lineTo(sx + 18, GROUND_Y + 6);
      ctx.stroke();
    } else {
      // meadow grass tufts
      ...
      if (hash > 0.82) {
        ...
        ctx.beginPath();
        ctx.arc(sx + 14, GROUND_Y - 3, 2.5, 0, Math.PI * 2);
        ctx.fill();
      }
    }
  }
...
function drawDecor() {
  for (const d of world.decor) {
    ...
    if (biome === "desert") {
      ...
      // tiny floating turret
      ctx.fillStyle = "#e8e0ff";
      ctx.fillRect(-8, -34, 16, 26);
      ...
      // glowing mushroom
      const pulse = 0.6 + 0.4 * Math.sin(frameCount * 0.08 + d.x * 0.01);
      ctx.fillStyle = "#3a5a55";
      ctx.fillRect(-2, -14, 4, 14);
      ctx.fillStyle = `rgba(46,230,199,${pulse})`;
      ctx.beginPath();
      ctx.ellipse(0, -16, 10, 7, 0, 0, Math.PI * 2);
      ctx.fill();
    } else if (biome === "underwater") {
      // swaying seaweed / coral
      const sway = Math.sin(frameCount * 0.05 + d.x * 0.02) * 4;
      ctx.strokeStyle = "#2e8b6f";
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.moveTo(0, 0);
      ctx.quadraticCurveTo(sway, -18, 0, -32);
      ctx.stroke();
      ctx.fillStyle = "#ff8fa3";
      ctx.beginPath();
      ...
      ctx.fill();
      const flicker = 0.5 + 0.5 * Math.sin(frameCount * 0.3 + d.x);
      ctx.fillStyle = `rgba(255,112,67,${flicker})`;
      ctx.beginPath();
      ctx.arc(0, -10 - flicker * 6, 4, 0, Math.PI * 2);
      ctx.fill();
    } else {
...
function drawSpikes() {
  const SPIKE_COLORS = {
    desert: "#4a7c3f", cave: "#9d7fd6", snow: "#b8e2f2",
    castle: "#8a5a5a", cloud: "#d4c4f0", hollow: "#2ee6c7",
    underwater: "#4a2f5c", volcano: "#1a1010",
  };
  for (const s of world.spikes) {
    const sx = s.x - camX;
    if (sx < -40 || sx > W + 40) continue;
    const biome = currentBiomeAt(s.x);
    ctx.fillStyle = SPIKE_COLORS[biome] || "#e57373";
...
function drawSparkles() {
  for (const sp of sparkles) {
    ...
    const alpha = Math.max(0, Math.min(1, sp.timer / (sp.maxTimer || 30)));
    ctx.fillStyle = sp.color ? `rgba(${sp.color},${alpha})` : `rgba(255, 213, 79, ${alpha})`;
    ctx.fillRect(sxp - (sp.size || 3), sp.y - (sp.size || 3), (sp.size || 3) * 2, (sp.size || 3) * 2);
  }
}

function spawnBurst(x, y, colorRgb, count) {
...
function drawBoss() {
  ...
  if (boss.isFinal) {
    ...
    const cy = boss.y + boss.h / 2;
    if (boss.type === "bat") {
      const flap = Math.sin(frameCount * 0.3) * 10;
      ctx.fillStyle = flashOn ? "#ffffff" : "#3a2f4a";
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.lineTo(cx - boss.w * 0.55, cy - 10 - flap);
      ctx.lineTo(cx - boss.w * 0.2, cy + 4);
      ctx.closePath();
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ...
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = flashOn ? "#ffffff" : "#5c4a7a";
      ctx.beginPath();
      ctx.ellipse(cx, cy, boss.w / 3.2, boss.h / 2.6, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = "#c0453a";
      ctx.fillRect(cx - 9, cy - 6, 5, 4);
      ctx.fillRect(cx + 4, cy - 6, 5, 4);
    } else if (boss.type === "golem") {
      ctx.fillStyle = flashOn ? "#ffffff" : "#6b6358";
      ctx.fillRect(sx, boss.y, boss.w, boss.h);
      ctx.fillStyle = "#4a443c";
      ctx.fillRect(sx, boss.y, boss.w, 8);
      ctx.fillRect(sx, boss.y + boss.h - 8, boss.w, 8);
      ctx.fillStyle = "#8a7f6a";
      ctx.fillRect(sx + 10, boss.y + 12, 12, 12);
      ctx.fillRect(sx + boss.w - 22, boss.y + 12, 12, 12);
      ctx.fillStyle = "#7dd6c8";
      ctx.fillRect(cx - 12, boss.y + boss.h * 0.4, 8, 6);
      ctx.fillRect(cx + 4, boss.y + boss.h * 0.4, 8, 6);
    } else {
      ...
      ctx.beginPath();
      ctx.moveTo(cx - 18, boss.y - 4);
      ctx.lineTo(cx, boss.y - 30);
      ctx.lineTo(cx + 18, boss.y - 4);
      ctx.closePath();
      ctx.fill();
    }
    if (boss.enraged) {
      ctx.fillStyle = `rgba(255,60,40,${0.15 + 0.1 * Math.sin(frameCount * 0.3)})`;
      ctx.beginPath();
      ctx.arc(cx, cy, Math.max(boss.w, boss.h) * 0.7, 0, Math.PI * 2);
      ctx.fill();
    }
    // HP pips
    for (let i = 0; i < boss.maxHp; i++) {
      ...
      ctx.beginPath();
      ctx.arc(cx - (boss.maxHp - 1) * 10 + i * 20, boss.y - 40, 6, 0, Math.PI * 2);
      ctx.fill();
    }
  }
...
function render() {
  ...
  if (shakeTimer > 0) {
    const s = shakeMag * (shakeTimer / 10);
...
document.getElementById("btn-settings-back").addEventListener("click", closeSubmenu);

const ambientSlider = document.getElementById("ambient-volume");
const sfxSlider = document.getElementById("sfx-volume");
ambientSlider.addEventListener("input", () => {
  ensureAudio();
  ambientVolumeMult = ambientSlider.value / 100;
  applyBiomeAmbience(effectiveBiome);
});
sfxSlider.addEventListener("input", () => {
  ensureAudio();
  sfxVolumeMult = sfxSlider.value / 100;
});

// --- Save code export/import ---
const savecodeModal = document.getElementById("screen-savecode");
const savecodeText = document.getElementById("savecode-text");
const savecodeLabel = document.getElementById("savecode-label");
const savecodePrimary = document.getElementById("savecode-primary");
let savecodeMode = "export";

document.getElementById("btn-export-save").addEventListener("click", () => {
  savecodeMode = "export";
  savecodeLabel.textContent = "Your save code — copy and store it somewhere safe:";
  savecodeText.value = btoa(JSON.stringify(save));
  savecodeText.readOnly = true;
  savecodePrimary.textContent = "Copy";
  savecodeModal.classList.remove("hidden");
});
document.getElementById("btn-import-save").addEventListener("click", () => {
  savecodeMode = "import";
  savecodeLabel.textContent = "Paste your save code:";
  savecodeText.value = "";
  savecodeText.readOnly = false;
  savecodePrimary.textContent = "Load";
  savecodeModal.classList.remove("hidden");
  savecodeText.focus();
});
document.getElementById("savecode-cancel").addEventListener("click", () => {
  savecodeModal.classList.add("hidden");
});
savecodePrimary.addEventListener("click", () => {
  if (savecodeMode === "export") {
    savecodeText.select();
    try {
      navigator.clipboard.writeText(savecodeText.value);
      showToast("Copied!");
    } catch (e) {
    ...
    try {
      const parsed = JSON.parse(atob(savecodeText.value.trim()));
      ...
      showToast("That code doesn't look valid");
    }
  }
});
...
function answerQuestion(idx) {
  if (idx === currentQuestion.c) {
    ...
    playCorrectSound();
    spawnBurst(player.x + player.w / 2, player.y, "255,213,79", 14);
    gameState = "playing";
    hideAllScreens();
    refillQuestionQueue();
  } else {
